...
void calculateStats(const std::vector<float>& data, std::vector<float>& metrics)
{
  ...
  metrics.push_back(average);
  metrics.push_back(variance);
  metrics.push_back(kurtosis);
  metrics.push_back(skew);
...
void loop()
{
  ...
  calculateStats(axData, metrics);

  Serial.println(F("Aceleración en el eje y:"));
  calculateStats(ayData, metrics);

  Serial.println(F("Aceleración en el eje z:"));
  calculateStats(azData, metrics);

  Serial.println(F("Velocidad angular en el eje x:"));
  calculateStats(gxData, metrics);

  Serial.println(F("Velocidad angular en el eje y:"));
  calculateStats(gyData, metrics);

  Serial.println(F("Velocidad angular en el eje z:"));
  calculateStats(gzData, metrics);

  // Imprimir el vector de métricas
  Serial.println(F("Vector de métricas:"));
